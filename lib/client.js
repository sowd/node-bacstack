...
class Client extends EventEmitter {
  ...
  _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length, port) {
    let result;
    ...
    if (service === baEnum.UnconfirmedServices.SERVICE_UNCONFIRMED_I_AM) {
       ...
       * });
       */
      this.emit('iAm', {address: address, deviceId: result.deviceId, maxApdu: result.maxApdu, segmentation: result.segmentation, vendorId: result.vendorId, port: port});
  ...
  _handlePdu(address, type, buffer, offset, length, port) {
    let result;
    ...
    switch (type & baEnum.PduTypes.PDU_TYPE_MASK) {
      case baEnum.PduTypes.PDU_TYPE_UNCONFIRMED_SERVICE_REQUEST:
        ...
        this._processUnconfirmedServiceRequest(address, result.type, result.service, buffer, offset + result.len, length - result.len, port);
        break;
  ...
  _handleNpdu(buffer, offset, msgLength, remoteAddress, remotePort) {
    // Check data length
    ...
    this._handlePdu(remoteAddress, apduType, buffer, offset, msgLength, remotePort);
  }

  _receiveData(buffer, remoteAddress, remotePort) {
    // Check data length
    ...
    if (result.func === baEnum.BvlcFunctions.BVLC_ORIGINAL_UNICAST_NPDU || result.func === baEnum.BvlcFunctions.BVLC_ORIGINAL_BROADCAST_NPDU || result.func === baEnum.BvlcFunctions.BVLC_FORWARDED_NPDU) {
      this._handleNpdu(buffer, result.len, buffer.length - result.len, remoteAddress, remotePort);
    } else {
