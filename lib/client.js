...
class Client extends EventEmitter {
  ...
  _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length) {
    ...
    if (service === baEnum.UnconfirmedServices.SERVICE_UNCONFIRMED_I_AM) {
      ...
      /**
       * The whoIs event represents the request for an IAm response to detect all devices in a BACNET network.
   ...
   * @param {function} next - The callback containing an error object in case of a failure.
   * @example
   * const bacnet = require('bacstack');
   * const client = new bacnet();
   *
   ...
   * const client = new bacnet();
   *
   * client.reinitializeDevice('192.168.1.43', bacnet.enum.ReinitializedStates.BACNET_REINIT_COLDSTART, (err) => {
   *   console.log('err: ', err);
   * });
   */
